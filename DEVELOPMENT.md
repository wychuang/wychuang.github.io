...
- `assets/`：简笔插画头像、两校官网校徽的透明底等尺寸版本与经过公开边界检查的项目截图；同一项目的结果、细节与过程画面使用明确文件名区分。
...
- 浙江大学校徽取自[浙江大学校标规范页](https://www.zju.edu.cn/xb/list.htm)，清华大学校徽取自[清华大学视觉形象识别系统](https://vi.tsinghua.edu.cn/gk/xxbz/xh.htm)。保留官网配色和比例，只清理白色画布、统一透明安全区并按校徽外圈等比缩放。
...
- Preserve the official colors and proportions of the ZJU and Tsinghua emblems; remove only the white canvas, normalize the transparent safe area, and scale both outer rings to one optical diameter.
